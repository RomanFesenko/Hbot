...
class CEnemyData
{
   ...
   private:
   const CBot& m_owner;// the bot that owns the data
   const player_t& m_enemy;
   int m_visible=0;
   palyer_data_t m_last_enemy_data;// where the enemy is last seen
   palyer_data_t m_last_owner_data;
   float m_last_time_vis=0; // when the enemy is last seen
...
// true if 1st is less dangerous than 2nd
...
class CDamageRegister
{
    ...
    // last damage was done by an invisible enemy
    // not more than time seconds ago
