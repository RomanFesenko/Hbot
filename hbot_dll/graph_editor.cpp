...
//CMapGraphViewer::ShowNode-отрисовка нода графа
void CMapGraphViewer::ShowNode(const CWayPoint& wp,int id_style)
{
    // определяем стиль рисования по идентификатору
    ...
    //определяем что нужно: рисовать,перерисовывать, или ничего не нужно
...
//CMapGraphViewer::ShowNode-отрисовка ребра графа
...
// CMapGraphViewer::ShowItem - указание айтема
...
// Установление типа стиля отрисовки нодов и ребер разных видов
...
// CGraphRenderer -колбэк для периодической перерисовки
// нодов, отфильтрованых по определенному признаку.

// Нужен, т.к HL не позволяет отрисовать полностью
// достаточно крупный граф из за ограничений на число Entity-ей
...
// установление связи данного типа между маркироваными нодами
...
std::string CMapGraphEditor::SetEdge(int type_12,int type_21)
{
    ...
    // прячем прежнее ребро если таковое было
    if(iter)
    {
        glMapGraphViewer.HideEdge(iter.value());
    }
...
std::string CMapGraphEditor::Run(float max_beams,const player_t&player)
{
    ...
    {
        1.0f,// перерисовка с периодом 1.0 сек.
        redrawer
    });
    assert(add_res);
...
/* BuildDefaultGraph-по данному множеству нодов N и ограничений
   на связность строит граф по умолчанию.
 Алгоритм:
 1) Строится граф Gv на N - между Ni и Nj устанавливается связь
  если между  Ni и Nj есть прямая видимость.
 2) На графе Gv ищется минимальное остовное дерево MST,все
 ребра не принадлежащие MST удаляются -> получаем GvMST.
 3) GvMST насыщается ребрами функцией CMapGraph::UpgradeAdjacency
*/
                                        ...
                                        float min_angle)
{
    ...
    // минимальное остовное дерево гарантирует связность
    // полученого графа
    std::vector<std::pair< CWayPoint*, CWayPoint*>> edges;
    edges.reserve(glMapGraph.NumWP());
    edges.clear();
    // сохраняем результат работы алгоритма крускала в виде
    // пар связанных нодов
    ...
    //насыщаем допустимыми ребрами полученое остовное дерево
...
//динамическая отрисовка ближайшего к игроку фрагмента графа
//с использованем не более чем max_beams лучей
...
void CNearestRenderer::Rendering()
{
    ...
    //отрисовка лучей указывающих направление обзора для валидного графа
...
//динамическая отрисовка пути в goal из
// местоположения игрока c ограничителями
