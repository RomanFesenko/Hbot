...
// ItemPriority-determining the value of an item on a range of 0 ... 1
...
///CEnemyData -
// save and update information about the enemy
// Allows you to get rid of multiple calls computationally expensive
// functions for determining the visibility of the enemy
...
//DefaultEnemyCompare- function of comparing opponents by degree
// their dangers
...
///CEnemyDataPool
// save and update information about opponents
// with the definition of the most dangerous
...
// DispatchNewCurrent - returns true if
// the most dangerous enemy has changed
...
// PriorityViewPoint-where to look / shoot
...
// LookBackViewing - returns where and how to look if
// damage has been done but the enemy is not visible
...
// fn_shooting_action-is it necessary to shoot / reload
// depending on the state of the weapon and aiming accuracy
...
/// CDamageRegister
// Logs damage, the executable state can react
...
/// CExtUseWall
// Extension of CUseWall with reaction to inflicted damage
...
// CExtUseWall :: j_viewer - look around if there was
// damage done
...
/// CVisitedItemsPoints
// logs recently visited CItemPoint
// so that the bot does not return to empty space
...
///RangeNeededItems
// array [0] - the most valuable items, array [2] - the least
...
/// CPriorityItemSearch
// Visitor to Dijkstra's algorithm, stopping the algorithm
// if an item with the highest priority is found,
// and registering a couple of nearest nodes
// lowest priority
...
// RouteToPriorityItems - Returns path parameters
// and CItemPoint to the nearest valuable item
...
// CDeathActivity - click the left mouse button to respawn
...
///CTotter
// swing from side to side, preventing aiming
...
/// CRoute
// Extension CBaseRoute including
// most informative looking around
// and provides a simple interface for CMainTask
...
// set a route to the most needed item
// or a random route if nothing is needed or nothing
...
/* CRoute :: UpdateCurrent - if the current destination node
 has not changed, but the bot has lost the shortest path -
 all shortest paths to the target node are searched:
 finding all shortest paths FROM the target node in the reversed graph
 finds all shortest paths to the target node in the source graph
 and installing a new short. way
*/
...
// CRoute::SetToVector- the way to a specific point
...
m_totter(main_task.m_enemies)
{
    SetJumpTable(&m_route,id_enemy_vis_st,
                {{
                    id_enemy_vis_st,// enemy is visible
                    [this](CTask*task){return j_from_vis_enemy(task);}
                },
                {
                    id_enemy_hide_st, // enemy disappeared
                    [this](CTask*task){return j_from_hide_enemy(task);}
                },
                {
                    id_check_damage, // damage has been done, but the enemy is not visible
                    [this](CTask*task){return j_from_check_damage(task);}
                },
                {
                    id_totter_st, // the closest approach to the visible enemy
...
// CPursuit :: m_UpdateRoute- update the pursuit route
// if the distance between the current position of the enemy
// and the target point is greater than m_max_for_update
...
// CPursuit :: j_from_vis_enemy - if the enemy is visible
// attack until the distance is reduced to m_min_needed_dist
...
// CPursuit :: j_from_hide_enemy - if the enemy hid
// run to the point where it was last seen
// also need to react to the damage done
...
// CPursuit :: j_from_check_damage - look back if there was damage
// keep chasing
...
// Extend CRoute to target including
// shootouts with a visible enemy and reaction to damage
...
//CMainTask::j_from_spawn- respawn
...
// CMainTask::j_from_use_wall- use of "ATMs"
...
/// DefBattleBalanse
// Determining the balance of power when entering a battle
