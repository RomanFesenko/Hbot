...
//executor_t::m_TurnVelocity - скорость поворота
// в зависимости от разности между текущим и желательным углом обзора

float executor_t::m_TurnVelocity(float delta_ang)const
{
    if(delta_ang>0.0f)
    {
        return (delta_ang>60.0f)? 360.0f:m_bot.m_chars.turn_velocity;//250-норм
    }
    else
    {
...
//executor_t::EvalViewAngle -выч. устанавливаемый угол обзора
// в зависимости от текущего и желательного угла обзора
...
//CTask::ForbidRedef - запрет на переопределение реакции
...
///CFSMTask
// Иерархический конечный автомат, состояния которого являются CTask
// которые также могут быть CFSMTask,произвольной степени вложености
// CFSMTask::SetJumpTable - установление таблицы переходов
// каждому состоянию соответствует своя функция перехода
...
//CFSMTask::Update() - осущ.переходов в самом автомате
// и во всех вложеных

void CFSMTask::Update()
{
    m_current->Update();// вначале переходы во всех вложеных
    ...
    // возможно изменение состояния без
    // изменения исполняемой CTask
    ...
    // m_jump_action - действие, выполняемое при переходе
    // в другое состояние или при изменении исполняемой CTask,
    // например посылка собщений
...
//CFSMTask::SetForced - принудительное установление
// исполняемой CTask и текущего состояния
...
//Каждый запрос GetMove,GetActivity,GetView
//линейно зависит от глубины вложености CFSMTask
...
///CTaskChain
// последовательное исполнение серии CTask
...
///CMoveChecker
// отслеживает среднюю за m_period_check скорость движения
// к m_goal
...
///CViewer
// последовательное исполнение серии обзоров
// куда смотреть в течение какого времени
...
///CMoveToPoint
// движение к фиксированой точке с отслеживанием
// скорости движения

CMoveToPoint::CMoveToPoint( )
{
    ForbidRedef(id_redef_move_t);//нельзя переопределять способ движ.
}
...
///CMoveOnStairs
// движение по отвесной лестнице, алгоритм основан
// на удерживании небольшого  расстояния между
// ботом и линией соединяющей нач. и конеч. точки
...
///COvercomeObstacle
// преодоление препятствия на пути к m_data.to_vec,
// алгоритм основан на добавлении небольшой
// перпендикулярной составляющей к текущему
// вектору m_data.to_vec-bot.Center()
...
///CBaseRoute
// базовая CTask выполняющая движение из m_vec_from в m_vec_to
// вдоль набора заданых ребер CMapGraph

CBaseRoute::CBaseRoute()
{
    SetJumpTable(&m_base_move,id_to_start_node_st,
            {{
                // движение из стартовой точки к первому ребру
                id_to_start_node_st,
                [this](CTask*task)
                 {return jump_from_start_node(task);}
            },
            {
                // движение вдоль ребра графа
                id_on_route_st,
                [this](CTask*task)
                {return jump_from_on_route(task);}
            },
            {
                // что делать если где то застряли
                id_in_obstacle_st,
                [this](CTask*task)
                {return jump_from_in_obstacle(task);}
            },
            {
                // движение по отвесной лестнице
                id_staircase_st,
                [this](CTask*task)
                {return jump_from_on_route(task);}
            },
            {
                // действия по пользованию подъемником
                id_on_elevator_st,
                [this](CTask*task)
                {return jump_from_on_route(task);}
            },
...
// CBaseRoute::m_JumpMessage - посылка сообщения абстрактному обработчику
// m_tracker при переходе с одного ребра пути на другое
...
std::pair<CTask*,int> CBaseRoute::update_route_task()
{
    ...
    if(!m_path_gen->empty())// сдедущие ребро пути
    {
...
///CRouteViewer
// способ обзора местности при исполнении CBaseRoute

CRouteViewer::CRouteViewer()
{
    SetJumpTable(&m_viewer,id_view_point,
                {{
                    // смотрение в конечную точку ребра
                    // вдоль которого бот двигается сейчас
                    id_view_point,
                    [this](CTask*task)
                ...
                {
                    // довыполнение уже неактульного смотрения
                    id_incomplete_view,
                    [this](CTask*task)
...
// движение в фиксированом направлении и в фиксированую точку
...
// удерживание заданой максимальной дистанции до vec
...
// смотрение в фиксированом направлении и в фиксированую точку
...
// функция обзора может заимствоваться из другой темы
...
// завершение темы по прошествию фиксированого времени
...
// завершение темы по достижению заданой точки
...
// завершение темы по достижению заданого угла обзора
// или направления взгляда
...
///CUseButton
// Базовая функция использования кнопок

CUseButton::CUseButton():
m_use_button(this)
{
    SetChain({{&m_set_view,id_set_view_st},// направить взгляд на кнопку
             { &m_use_button,id_use_button_st}});// IN_USE кнопку

    // нельзя переопределять ни обзор ни движение
...
//условие завершения определяется производными классами
...
//RouteToVector- маршрут из from_vec в to_vec
...
//RouteToVector- маршрут в случайный нод граф из текущего местоположения
