...
//CBaseNode- базовый класс в модели необычно рекурсивного шаблона:
//class CDerivedNode:public CBaseNode<some_edge_t,CDerivedNode>
// Производный класс получает интерфейс CBaseNode для пользования
// графопостроителем CGraphComposer
...
class CBaseNode
{
   ...
   // delete_edge,set_edge -  доступны
   // лишь для CGraphComposer
   ...
   // способ исключительного для CBaseNode доступа к закрытым частям
   // CBaseNodeIterator  через internal_iterator_data,
   // нужен из-за запрета C++ на объявление 2х классов друзьями друг друга
   // см. конструктор CBaseNodeIterator(internal_iterator_data)
   struct internal_iterator_data
   {
       using difference_type=typename decltype(m_adjacents)::difference_type;
       const node_t* base;
       difference_type index;
       explicit internal_iterator_data(const node_t* _base,difference_type _ind):
       base(_base),index(_ind){}
   };
   internal_iterator_data get_iterator_data(const node_t& other)const
   {
       auto adj_iter=std::lower_bound(m_adjacents.begin(),
                                      m_adjacents.end(),
                                      other,_adj_comp_node());
       ...
       {
           return internal_iterator_data(static_cast<const node_t*>(this),
                                         adj_iter-m_adjacents.begin());
       }
       else
       {
           return internal_iterator_data(nullptr,0);
       }
   }
   public:
...
// CBaseNodeIterator
// Предоставляет минимально необходимый интерфейс для
// стандартных алгоритмов на графах
template<class n_t>
class CBaseNodeIterator
{
    ...
    using adj_iterator=typename node_t::adj_iterator;
    ...
    //этот конструктор может вызываться лишь из членов node_t
    explicit CBaseNodeIterator(typename node_t::internal_iterator_data it_d)
    {
        m_nodefrom=const_cast<node_t*>(it_d.base);
        m_adj_iterator=m_nodefrom->m_adjacents.begin()+it_d.index;
    }
    ...
    /*node_t::adj_size_type size_adjacents()const
    {
        return m_nodefrom->size_adjacents();
    }*/

    // обратное ребро - совпадает с edge() для неориентированого ребра
    // nullptr для ориентированого, !=для мультиориентир.
    edge_t* get_reverse_edge()const
    {
        return to_node().get_edge_to(from_node());
    }
    // итератор для обратного ребра
...
//get_iterator_to
// Сложность  - логарифмическая от числа исходящих ребер this
...
{
    internal_iterator_data iter_data=get_iterator_data(other);
    if(iter_data.base!=nullptr)
    {
        return {iterator(iter_data)};
    }
    else
    {
        return {};
...
//CGraphComposer
// Отвечает за построение и изменение графа, шаблонный node_t
// должен предоставлять интерфейс CBaseNode
...
class CGraphComposer
{
   ...
   //////////////////////////////////////////////////////////////////
   // Создание и разрушение связей между нодами.                  ///
   // Конечный результат НЕ зависит от предыдущего состояния связи///
   // return- тип предыдущей связи.                                //
   //////////////////////////////////////////////////////////////////
   // неориентированое ребро
   ...
   // ориентированое ребро от fst к snd
   ...
   // пара ориентированых ребер связывающих 2 нода
   ...
   // разрушение любой связи между нодами
