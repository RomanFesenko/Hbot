...
// edges with negative weight are allowed
// returns false if there are negative cycles
template<class node_t>
template<class _Restr>
bool bellman_ford_process<node_t>:: done(const node_t& start_node,_Restr& restrictor)
{
   ...
   while(true)
   {
       ...
       if(next_for_process==nullptr)//signal of the end of the current processed series
       {
           if(++num_series>num_nodes()) return true;
           for_process.push_back(nullptr);
           continue;
       }
       // processing adjacent nodes - standard relaxation algorithm
       // with the difference that if you can reduce
       // some path on  m_num_nodes iteration, it is a negative loop
       iterator temp_it;
       temp_it.set_base(*next_for_process);
       for(temp_it.begin();!temp_it.end();temp_it.next())
       {
           // perhaps the edge is hidden
           ...
           {
               //possibly an adjacent node is hidden
               if constexpr(def_node_discover<_Restr,node_t>)
               {
           ...
           else if(m_nodes_state(adj_for_process)==Alive)// relaxation
           {
               // if after processing m_num_nodes-1
               // series comes across the opportunity to weaken the edge - cycle
