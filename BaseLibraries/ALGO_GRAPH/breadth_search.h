...
// наличие обработчика вновь посещенных но уже обработаных ребер
// нужен например при проверке ацикличности графа
...
class breadth_search
{
    ...
    //минимальное колво ребер на пути от исходного узла
    TProperty<size_type,node_t> m_distance;
    // итератор - ребро ведущее из предыдущей вершины
    TProperty<iterator,node_t> m_prev_iterator;//prev_node[node.index()] - предыдущий дл€ node
...
//breadth_search
//return - колво нодов достижимых из start_node
...
breadth_search<node_t>::done(const node_t& start_node,_Restr& restrictor,bool freeze_state)
{
    ...
    // страховка от неправильного объ€влени€ визиторов в restrictor-
    ...
    // между вызовами  не должно мен€тьс€
    // число узлов графа
    assert(m_num_nodes==m_graph_nodes.size());

    // сохран€ть или нет результаты предыдущего вызова
    if (!freeze_state)
    {
    ...
    while(!queue.empty())
    {
        ...
        // обработка всех исход€щих ребер
        ...
        for(it.begin();!it.end();it.next())
        {
            ...
            if(m_nodes_state(to_n)==Sleep)//сп€щий нод, будим
            {
                if constexpr(def_node_discover<_Restr,node_t>)
                {
                    // фильтруем узел
...
// обработка всех св€зных компонент неориентированого графа
