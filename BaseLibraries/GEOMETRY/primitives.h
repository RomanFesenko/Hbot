...
class FixedVector
{
    ...
    _Der& operator*=(T2 val)
    {
        ...
        return *(static_cast<ptr_der_class_t>(this));
    }
    template<class T2>
    _Der operator*(T2 val) const
    {
        _Der res;
        for(int i=0;i<_Dim;++i) res[i]=m_data[i]*val;
        return res;
    }
    template<class T2>
    _Der& operator/=(T2 val)
...
// “ребуютс€ перегруженные операции +,-,/,* дл€ типа point_t
// ѕостроение окружности по 3 точкам на ней
...
->std::pair<point_t,decltype(pt1[0]+0)>
{
    ...
    res.second=len12/(2.0*fabs(cs.second)); // радиуc
    res.first=(pt1+pt2)/2.0+coeff*turn_versus_clockwise(d12);
    return res;
}

//////////////////////////////////////////////////////////////
// Ћучи и св€занные с ними алгритмы                       //
//////////////////////////////////////////////////////////////

template<class point_t>
struct line_2D_t // направленна€
{
    point_t base_point;
...
// определение рассто€ние от точки до пр€мой
...
// генераци€ случайного массива num точек расположеных
...
// поиск минимального пр€моугольника содержащего все точки
// возвращаетс€ нижн€€ лева€ и верхн€€ права€
template<class point_t>
std::pair<point_t,point_t> close_rectangle(const std::vector<point_t>& arr)
{
    auto xcoord=[](const point_t&pt1,const point_t&pt2){return pt1[0]<pt2[0];};
    auto ycoord=[](const point_t&pt1,const point_t&pt2){return pt1[1]<pt2[1];};
    auto xdiap=std::minmax_element(arr.begin(),arr.end(),xcoord);
    auto ydiap=std::minmax_element(arr.begin(),arr.end(),ycoord);
    return {point_t((*(xdiap.first))[0],(*(ydiap.first))[1]),
    point_t((*(xdiap.second))[0],(*(ydiap.second))[1])};
}

// ќпределени€ точек пересечени€ геом примитивов

// ѕересечение 2 пр€мых
...
// ѕересечение окружности и пр€мой
...
struct intersect_line_circle_result
{
    ...
    std::pair<value_type,point_t> point_2; // угол и собственно точка
};
...
(const std::pair<point_t,decltype(std::declval<point_t>()[0]+0)>& circle,const line_2D_t<point_t>& ln)
{
    ...
    if(dist>circle.second) return intersect_line_circle_result<point_t>();// не пересекает
    value_type angleA=acos(dist/circle.second);
    value_type angleD=(pos==Right)? (ln.angle+Mpi/2.0):(ln.angle-Mpi/2.0);
    normalize_angle(angleD);
    intersect_line_circle_result<point_t> res;
    res.is_intersect=true;
    // перва€ точка пересечени€
...
struct intersect_two_circles_result
{
    using value_type=decltype(std::declval<point_t>()[0]+0);
    bool is_intersect=false;
    // 2 угла дл€ обеих окр. и собственно точка
 ...
 const std::pair<point_t,decltype(std::declval<point_t>()[0]+0)>& circle2)
{
    ...
    value_type delta2=acos((D12*D12+R2*R2-R1*R1)/(2*R2*D12));
    {
        ...
        a2=Mpi+angleB+delta2;normalize_angle(a2);
...
// ‘ормирование базиса Ѕезье дл€ аппроксимации кривой
